...
///ENCONTRAR UNION
int padre[ MAX ]; //Este arreglo contiene el padre del inesimo nodo

//Metodo de inicializacion de los vertices
void MakeSet( int n ){
    for( int i = 1 ; i <= n ; ++i )
        padre[ i ] = i;
}
...
struct aristaa{
    ...
    float peso;
    aristaa(){}
    //Comparador por peso, me servira al momento de ordenar
    //lo realizara en orden ascendente

    bool operator<( const aristaa &e ) const {
        return peso < e.peso;
        //PARA OBTENER EL MAYOR, INVERTIR EL SIGNO
    }
}
